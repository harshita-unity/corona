...
export default function StateInterface(Props)
{ 
  ...
  const handleIcon=(event)=>{
   ...
   const handleClick=async()=>{
    ...
    formData.append("authorizedwebsite",authorizedWebsite)
    formData.append("icon",icon.bytes)
   
    

    var config={headers:{"content-type":"multipart/form-data"}}
    var result= await postDataAndImage('state/addnewstate',formData,config)
    if(result){
        swal({
    ...
    <div className={classes.subdiv}>
      <Grid container spacing={1}>
             ...
             <Grid item xs={12}>
               ...
               <Grid item xs={12}>
                  <TextField onChange={(event)=> setAuthorizedWebsite(event.target.value)} label="Authorizedwebsite" variant="outlined" fullWidth/>
                </Grid> 
