...
export default function NewsAgencyInterface(props)
{ const classes =useStyles();
    ...
    const handleClick=async()=>{
        ...
        if(isBlank(photoCopy.bytes)){
            error=true
            msg+="<font color='#e74c3c'><b>Plz select phtocopy... </b></font><br> "
        }
        ...
        var result= await postDataAndImage('newsagency/addnewnewsagency',formData,config)
        if(result){
            swal({
                title: "News Agency Record Submitted Successfully",
                icon: "success",
                dangerMode: true,
              })
        }
            ...
            <div className={classes.subdiv} >
                <Grid container spacing={1}>
                    ...
                    <Grid item xs={12} sm={6}>
                       <TextField onChange={(event)=>setAgencyName(event.target.value)} label="Agency Name" variant="outlined" fullWidth/>
                    </Grid>
